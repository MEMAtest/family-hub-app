...
School email intake is Gmail-based. From **Calendar -> Quick add & import**, connect the Google account that receives school mail, then forward a newsletter to the generated `ademolaomosanya+familyhub@gmail.com` address and press **Sync Gmail**. Family Hub reads only messages addressed to that plus-address, extracts selectable text from an attached PDF, applies the same date/event/subject classifier used by direct uploads, keeps uncertain items in review, deduplicates by Gmail message ID, stores supported attachments privately, and sends the normal in-app/push notification. Image-only PDFs remain available for download and can be uploaded directly for browser OCR. The sync searches the last 90 days and does not read the rest of the inbox.
