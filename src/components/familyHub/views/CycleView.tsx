...
import { BellRing, CalendarDays, Check, Clock3, Download, HeartPulse, Plus, ShieldCheck, Trash2, X } from 'lucide-react';
...
export const CycleView = () => {
  ...
  const [daily, setDaily] = useState<DailyCheckIn>({ logDate: todayKey(), flow: '', mood: '', energy: 3, painLevel: 0, sleepHours: 7, medication: '', notes: '', symptoms: [] });
  ...
  const load = async () => {
    ...
    try {
      ...
      setData(nextData);
      const existingToday = nextData.logs.find((log) => dateKey(log.logDate) === todayKey());
      if (existingToday) setDaily({ logDate: todayKey(), flow: existingToday.flow || '', mood: existingToday.mood || '', energy: existingToday.energy || 3, painLevel: existingToday.painLevel || 0, sleepHours: existingToday.sleepHours || 7, medication: existingToday.medication || '', notes: existingToday.notes || '', symptoms: existingToday.symptoms || [] });
  ...
  const savePeriod = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    try {
      const response = await fetch(`/api/families/${familyId}/cycles`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'period', startDate: values.get('startDate'), endDate: values.get('endDate'), notes: values.get('notes') }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || 'Could not save your period.');
      setShowPeriod(false);
      setNotice('Period saved privately.');
      await load();
    } catch (reason) {
  ...
  if (profileName && profileName.trim().toLowerCase() !== 'angela') return <div className="flex min-h-[60vh] items-center justify-center bg-[#f6f7f3] p-6 dark:bg-slate-950"><div className="max-w-sm text-center"><ShieldCheck className="mx-auto h-8 w-8 text-[#d8527d]" /><h1 className="mt-4 font-serif text-3xl">This is Angela&apos;s private area.</h1><p className="mt-3 text-sm text-slate-600 dark:text-slate-300">Health and cycle details stay with the profile they belong to.</p></div></div>;

  return <div className="min-h-full bg-[#fbf7f8] px-4 py-5 dark:bg-slate-950 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl">
    <header className="flex flex-wrap items-end justify-between gap-4 border-b border-[#eddde3] pb-5 dark:border-slate-800"><div><p className="text-sm font-semibold text-[#d8527d]">Angela&apos;s private health area</p><h1 className="mt-1 font-serif text-3xl">Health &amp; Cycle</h1><p className="mt-2 max-w-xl text-sm text-slate-600 dark:text-slate-300">A calm private timeline, gentle reminders, and your own wellbeing patterns.</p></div><button type="button" onClick={() => setShowPeriod(true)} className="inline-flex h-10 items-center gap-2 rounded-md bg-[#d8527d] px-3 text-sm font-semibold text-white hover:bg-[#bb3d65]"><Plus className="h-4 w-4" />Log period</button></header>
    ...
    {showPeriod && <div className="fixed inset-0 z-50 flex items-end bg-black/40 sm:items-center sm:justify-center sm:p-6"><form onSubmit={savePeriod} className="w-full max-w-md bg-white p-5 dark:bg-slate-900 sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold text-[#d8527d]">Private timeline</p><h2 className="font-serif text-2xl">Log a period</h2></div><button type="button" onClick={() => setShowPeriod(false)} aria-label="Close period form"><X className="h-5 w-5" /></button></div><div className="mt-5 grid gap-3"><label className="text-xs font-semibold text-slate-600 dark:text-slate-300">Start date<input aria-label="Start date" name="startDate" type="date" required defaultValue={todayKey()} className="mt-1 h-11 w-full rounded-md border border-slate-300 px-3 text-sm dark:border-slate-700 dark:bg-slate-950" /></label><label className="text-xs font-semibold text-slate-600 dark:text-slate-300">End date, optional<input aria-label="End date, optional" name="endDate" type="date" className="mt-1 h-11 w-full rounded-md border border-slate-300 px-3 text-sm dark:border-slate-700 dark:bg-slate-950" /></label><textarea name="notes" placeholder="Private note, optional" className="min-h-24 rounded-md border border-slate-300 p-3 text-sm dark:border-slate-700 dark:bg-slate-950" /></div><button disabled={busy} className="mt-5 h-11 w-full bg-[#d8527d] text-sm font-semibold text-white">Save private period</button></form></div>}
  </div></div>;
};
