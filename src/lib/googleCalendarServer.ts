...
export const encodeGoogleState = (familyId: string) => {
  const payload = Buffer.from(JSON.stringify({ familyId, ts: Date.now() })).toString('base64url');
...
export const decodeGoogleState = (state: string) => {
  ...
  const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as { familyId: string; ts: number };
